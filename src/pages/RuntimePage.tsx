...
import { buildWebhookEventList, KNOWN_WEBHOOK_EVENTS } from "../lib/webhookEvents";
...
export function RuntimePage() {
  ...
  const databaseValue = status.data?.database ?? ready.data?.database ?? "unknown";
  const databaseOk = databaseValue === "ok";
...
function ReadinessLine({ label, value, ok }: { label: string; value: string; ok: boolean }) { return <div><span>{label}</span><strong className="runtime-lines__value"><span className={`status-dot ${ok ? "status-dot--ok" : "status-dot--warn"}`} aria-hidden="true" />{value}</strong></div>; }
...
function WebhooksPanel() {
  ...
  const eventList = buildWebhookEventList(selectedEvents, customEvents);
  const create = useMutation({ mutationFn: () => api.request<WebhookReceipt>("/api/v2/admin/webhooks", { method: "POST", body: { url, events: eventList } }), onSuccess: async (result) => { setSecret(result.secret); setUrl(""); setSelectedEvents(["*"]); setCustomEvents(""); await queryClient.invalidateQueries({ queryKey: ["webhooks"] }); } });
  ...
  function toggleEvent(value: string) {
    setSelectedEvents((current) => {
      ...
      const rest = current.filter((item) => item !== "*");
