...
export function MemoryPage() {
  ...
  return <div className="page"><section className="page-intro"><div><p className="eyebrow">Context policy</p><h2>Memory should be useful, visible, and reversible.</h2><p>Review exactly what Helmora can retrieve, pin durable context, and exclude anything that no longer belongs.</p></div><Button label={showForm ? "Close form" : "Add memory"} variant={showForm ? "secondary" : "primary"} onClick={() => { setShowForm((value) => !value); }} /></section>
    ...
    <section className="panel"><div className="list-toolbar"><TextInput label="Filter memories" isLabelHidden value={search} onChange={setSearch} placeholder="Filter memory…" hasClear /><Badge variant="neutral" label={`${visible.length} memories`} /></div>{memories.error ? <RequestError error={memories.error} /> : memories.isPending ? <p className="muted-copy">Loading memory…</p> : visible.length ? <div className="memory-grid">{visible.map((memory) => <article className="memory-card" key={memory.id}><header><Badge variant={memory.pinned ? "teal" : "neutral"} label={memory.kind} /><span>{memory.pinned ? "Pinned" : memory.sensitivity}</span></header><p>{memory.content}</p><footer><time>{formatDate(memory.updatedAt)}</time><Button label="Exclude" variant="destructive" size="sm" onClick={() => { if (window.confirm("Exclude this memory from future retrieval?")) exclude.mutate(memory.id); }} /></footer></article>)}</div> : <p className="muted-copy">No active memories.</p>}</section>
  </div>;
}
