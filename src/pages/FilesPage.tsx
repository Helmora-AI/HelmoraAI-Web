...
export function FilesPage() {
  ...
  const clearInputValue = () => { if (input.current) input.current.value = ""; };
  const upload = useMutation({ mutationFn: async (file: File) => api.request<FileRecord>("/api/v2/files", { method: "POST", body: { filename: file.name, mediaType: file.type || "application/octet-stream", data: await fileToBase64(file) } }), onSuccess: async () => { setSelected(undefined); clearInputValue(); await queryClient.invalidateQueries({ queryKey: ["files"] }); }, onError: clearInputValue });
  ...
  return <div className="page"><section className="page-intro"><div><p className="eyebrow">Tenant storage</p><h2>Files enter once, then stay addressable.</h2><p>Uploads are tenant-scoped, content-hashed, and capped at 32 MiB before they can be attached to knowledge or model workflows.</p></div></section>
    <section className="panel upload-panel"><label className={dragging ? "file-drop file-drop--active" : "file-drop"} onDragOver={(event) => { event.preventDefault(); setDragging(true); }} onDragLeave={(event) => { if (!event.currentTarget.contains(event.relatedTarget as Node | null)) setDragging(false); }} onDrop={(event) => { event.preventDefault(); setDragging(false); setSelected(event.dataTransfer.files?.[0]); }}><input ref={input} type="file" onChange={(event) => { setSelected(event.target.files?.[0]); }} /><span>↑</span><div><strong>{selected?.name ?? "Choose or drop a file to upload"}</strong><small>{selected ? `${formatBytes(selected.size)} · ${selected.type || "application/octet-stream"}` : "Maximum 32 MiB per file"}</small></div></label><Button label="Upload to Hub" variant="primary" isLoading={upload.isPending} isDisabled={!selected || Boolean(invalid)} onClick={() => { if (selected) upload.mutate(selected); }} /></section>
    ...
    <section className="panel data-panel"><header className="panel__header"><div><p className="eyebrow">Stored objects</p><h3>Files</h3></div><Badge variant="neutral" label={`${files.data?.data.length ?? 0} files`} /></header><AsyncList error={files.error} isPending={files.isPending} loadingLabel="Loading files…">{files.data?.data.length ? <div className="file-list">{files.data.data.map((file) => <article key={file.id}><span className="file-type">{extension(file.filename)}</span><div><strong>{file.filename}</strong><small>{file.mediaType} · {formatBytes(file.bytes)} · {file.sha256.slice(0, 12)}…</small></div><Badge variant={file.status === "ready" ? "success" : "warning"} label={file.status} /><Button label="Download" variant="secondary" size="sm" onClick={() => { void download(file); }} /><Button label="Delete" variant="destructive" size="sm" onClick={() => { if (window.confirm(`Delete “${file.filename}”?`)) remove.mutate(file.id); }} /></article>)}</div> : <p className="muted-copy">No files uploaded.</p>}</AsyncList></section>
  </div>;
}
