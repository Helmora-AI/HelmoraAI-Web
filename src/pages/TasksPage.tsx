...
export function TasksPage() {
  ...
  return <div className="page"><section className="page-intro"><div><p className="eyebrow">Durable orchestration</p><h2>Background work you can inspect and stop.</h2><p>Tasks survive request boundaries, expose ordered events, and accept cancellation while queued or running.</p></div><Button label="Start research" variant="primary" href="/research" /></section>
    ...
    <div className="master-detail"><section className="panel master-list"><AsyncList error={tasks.error} isPending={tasks.isPending} loadingLabel="Loading tasks…">{tasks.data?.data.length ? <div className="record-list">{tasks.data.data.map((task) => <RecordRow key={task.id} mark="T" title={task.kind} subtitle={`${task.id} · ${formatDate(task.updatedAt)}`} active={task.id === selectedId} onClick={() => { setSelectedId(task.id); }} trailing={<StatusBadge status={task.status} />} />)}</div> : <p className="muted-copy">No tasks in this state.</p>}</AsyncList></section>
      <section className="panel detail-panel">{!active ? <div className="detail-empty"><span>T</span><h3>Select a task</h3><p>Ordered progress events and the terminal result will appear here.</p></div> : <><header className="detail-panel__header"><div><p className="eyebrow">{active.kind}</p><h3>{active.id}</h3><p>created {formatDate(active.createdAt)}</p></div><StatusBadge status={active.status} /></header><div className="progress" role="progressbar" aria-label="Task progress" aria-valuemin={0} aria-valuemax={100} aria-valuenow={Math.round(Math.max(active.status === "queued" ? 2 : 5, active.progress * 100))}><i style={{ width: `${Math.max(active.status === "queued" ? 2 : 5, active.progress * 100)}%` }} aria-hidden="true" /></div>{detail.error ? <RequestError error={detail.error} /> : null}<section className="task-events"><h4>Event timeline</h4><HelmoraScrollArea className="task-events__scroll" aria-label="Task event timeline">{liveEvents.map((event) => <article key={event.sequence}><span>{event.sequence}</span><div><strong>{event.type}</strong><small>{formatDate(event.createdAt)}</small></div><code>{JSON.stringify(event.payload)}</code></article>)}</HelmoraScrollArea></section>{active.result !== undefined || active.error !== undefined ? <section><p className="eyebrow">Terminal payload</p><JsonPreview value={active.result ?? active.error} className="helmora-scroll-surface" /></section> : null}{["queued", "running"].includes(active.status) ? <footer className="detail-panel__footer"><span /><Button label={active.cancelRequested ? "Cancellation requested" : "Cancel task"} variant="destructive" isDisabled={active.cancelRequested} isLoading={cancel.isPending} onClick={() => { if (window.confirm(`Cancel task “${active.id}”?`)) cancel.mutate(active.id); }} /></footer> : null}</>}</section>
