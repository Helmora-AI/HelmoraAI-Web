...
export function ResearchPage() {
  ...
  useEffect(() => {
    ...
    void (async () => {
      ...
      } catch (error) { if (!(error instanceof DOMException && error.name === "AbortError")) setTaskEvents((current) => [...current, "stream.error"]); }
    })();
  ...
  return <div className="page"><section className="page-intro"><div><p className="eyebrow">Retrieval workspace</p><h2>Search broadly. Fetch safely. Research durably.</h2><p>All outbound retrieval passes through Hub’s SSRF boundary, size limits, redirect policy, and configured search provider.</p></div></section>
    <div className="segmented" role="tablist"><button role="tab" aria-selected={mode === "search"} onClick={() => { setMode("search"); }}>Search web</button><button role="tab" aria-selected={mode === "fetch"} onClick={() => { setMode("fetch"); }}>Fetch URL</button><button role="tab" aria-selected={mode === "research"} onClick={() => { setMode("research"); }}>Deep research</button></div>
    ...
    {mode === "search" && searchResult ? <section className="panel research-results"><header className="panel__header"><div><p className="eyebrow">{searchResult.provider} · {searchResult.tookMs} ms</p><h3>{searchResult.results.length} search results</h3></div></header>{searchResult.results.map((result) => <article key={result.id}><span>{result.source.slice(0, 2).toUpperCase()}</span><div><a href={result.url} target="_blank" rel="noreferrer">{result.title}</a><p>{result.snippet}</p><small>{result.url}{result.publishedAt ? ` · ${result.publishedAt}` : ""}</small></div></article>)}</section> : null}
    ...
    {mode === "research" && task ? <section className="panel task-focus"><header><div><p className="eyebrow">Durable task</p><h3>{task.id}</h3></div><Badge variant={task.status === "completed" ? "success" : task.status === "failed" ? "error" : "info"} label={task.status} /></header><div className="progress"><i style={{ width: `${Math.max(4, task.progress * 100)}%` }} /></div><p>{taskEvents.length ? taskEvents.join(" → ") : "Waiting for worker events…"}</p>{task.result !== undefined ? <pre className="json-preview">{JSON.stringify(task.result, null, 2)}</pre> : null}</section> : null}
  </div>;
}
