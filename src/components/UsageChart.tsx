...
export default function UsageChart({ buckets, metric, height = 240 }: UsageChartProps) {
    ...
    return (
      <div className="usage-chart" role="img" aria-label={ariaLabel}>
        <ResponsiveContainer width="100%" height={height}>
          <AreaChart data={buckets} margin={{ top: 12, right: 8, bottom: 0, left: -12 }}>
            <defs>
            ...
            <ChartGrid />
            <ChartAxes />
            <Tooltip contentStyle={tooltipStyle} labelFormatter={formatDate} formatter={(value) => formatCompact(Number(value))} />
  ...
  if (metric === "latency") {
    return (
      <div className="usage-chart" role="img" aria-label={ariaLabel}>
        <ResponsiveContainer width="100%" height={height}>
          <LineChart data={buckets} margin={{ top: 12, right: 8, bottom: 0, left: -12 }}>
            <ChartGrid />
            <ChartAxes />
            <Tooltip contentStyle={tooltipStyle} labelFormatter={formatDate} formatter={(value) => [`${Math.round(Number(value))} ms`, "Average latency"]} />
  ...
  if (metric === "cost") {
    return (
      <div className="usage-chart" role="img" aria-label={ariaLabel}>
        <ResponsiveContainer width="100%" height={height}>
          <ComposedChart data={buckets} margin={{ top: 12, right: 8, bottom: 0, left: -12 }}>
            <ChartGrid />
            <ChartAxes />
            <Tooltip
              contentStyle={tooltipStyle}
              labelFormatter={formatDate}
              formatter={(value, name) => name === "Known estimate" ? [formatCost(Number(value)), name] : [Number(value), name]}
            />
            <Legend iconType="circle" iconSize={7} />
  ...
  return (
    <div className="usage-chart" role="img" aria-label={ariaLabel}>
      <ResponsiveContainer width="100%" height={height}>
        <BarChart data={buckets} margin={{ top: 12, right: 8, bottom: 0, left: -12 }}>
          <ChartGrid />
          <ChartAxes />
